    ...
    public static void handleParticle(IClientPlayNetHandler handler, WNSSpawnParticlePacket particle){
        if(handler instanceof ClientPlayNetHandler) {
            ...
            if (particle.getParticleCount() == 0) {
                ...
                try {
                    ((ClientPlayNetHandler) handler).world.addParticle(particle.getParticle(), particle.isLongDistance(), particle.getXCoordinate(), particle.getYCoordinate(), particle.getZCoordinate(), d0, d2, d4);
                ...
                for (int i = 0; i < particle.getParticleCount(); ++i) {
                    try {
                        ((ClientPlayNetHandler) handler).world.addParticle(particle.getParticle(), particle.isLongDistance(), particle.getXCoordinate(), particle.getYCoordinate(), particle.getZCoordinate(), particle.getXOffset(), particle.getYOffset(), particle.getZOffset());
