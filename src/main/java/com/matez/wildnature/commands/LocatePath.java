    ...
    public static int findTeleportBiome(CommandSource cs, ServerPlayerEntity player){
        ...
        if(player.getEntityWorld().getDimension() instanceof OverworldDimension) {
            ...
            if(path!=null){
                ...
                StringTextComponent s42 = new StringTextComponent(TextFormatting.AQUA + " - " + TextFormatting.GOLD + (int)Utilities.getDistance(new BlockPos(player.posX,player.posY,player.posZ),path) + TextFormatting.AQUA+" blocks away.");
                ...
                player.sendStatusMessage(new StringTextComponent(TextFormatting.GREEN+"Found path " + TextFormatting.AQUA+ (int)Utilities.getDistance(new BlockPos(player.posX,player.posY,player.posZ),path) + TextFormatting.GREEN+" blocks away"),true);
