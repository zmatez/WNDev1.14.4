...
public final class WNWorldEntitySpawner {
                        ...
                        label115: {
                            if (k2 < i2) {
                                label123: {
                                    ...
                                    if (canSpawn == -1 || (canSpawn == 0 && (d0 > 16384.0D && mobentity.canDespawn(d0) || !canSpawn(mobentity,p_222263_1_, SpawnReason.NATURAL) || !mobentity.isNotColliding(p_222263_1_)))) {
                                        break label115;
                                    }
                    ...
                    for(int i2 = 0; !flag && i2 < 4; ++i2) {
                        ...
                        if (biome$spawnlistentry.entityType.isSummonable() && canCreatureTypeSpawnAtLocation(EntitySpawnPlacementRegistry.PlacementType.ON_GROUND, worldIn, blockpos, biome$spawnlistentry.entityType)) {
                            ...
                            if (entity instanceof MobEntity) {
                                ...
                                if (canSpawn(mobentity,worldIn, SpawnReason.CHUNK_GENERATION) && mobentity.isNotColliding(worldIn)) {
    ...
    public static boolean canSpawn(MobEntity entity, IWorld worldIn, SpawnReason spawnReasonIn) {
        if(entity instanceof AnimalEntity && !(entity instanceof TurtleEntity) && !(entity instanceof MooshroomEntity)) {
            Block b = worldIn.getBlockState(entity.getPosition().down()).getBlock();
            boolean isInGroup = b==Blocks.GRASS_BLOCK || b== WNBlocks.DRIED_GRASS_BLOCK;

            return (isInGroup ? 10.0F : worldIn.getBrightness(entity.getPosition()) - 0.5F) >= 0.0F;
        }


        return entity.canSpawn(worldIn,spawnReasonIn);
    }
}