...
public abstract class SimplexChunks<T extends GenerationSettings> extends ChunkGenerator<T> {
    ...
    private final SimplexOctaveGenerator simplexOctaveX,simplexOctaveZ,simplexOctaveY;
    private final INoiseGenerator surfaceDepthNoise;
    protected final BlockState defaultBlock;
    protected final BlockState defaultFluid;

    public SimplexChunks(IWorld worldIn, BiomeProvider biomeProviderIn, int p_i49931_3_, int p_i49931_4_, int p_i49931_5_, T p_i49931_6_, boolean usePerlin) {
        ...
        simplexOctaveX = new SimplexOctaveGenerator(worldIn.getSeed(),16);
        simplexOctaveZ = new SimplexOctaveGenerator(worldIn.getSeed(),16);
        simplexOctaveY = new SimplexOctaveGenerator(worldIn.getSeed(),8);
        simplexOctaveX.setScale(1/64);
        simplexOctaveZ.setScale(1/64);
    ...
    private double useNoises(int chunkX, int chunkY, int chunkZ, double coordinateScale, double heightScale, double depthSize, double unk1) {
        ...
        for(int i = 0; i < 16; ++i) {
            double d4 = OctavesNoiseGenerator.maintainPrecision((double)chunkX * coordinateScale * d3);
            double d5 = OctavesNoiseGenerator.maintainPrecision((double)chunkY * heightScale * d3);
            double d6 = OctavesNoiseGenerator.maintainPrecision((double)chunkZ * coordinateScale * d3);
            double d7 = heightScale * d3;
            d0 += this.field_222568_o.getOctave(i).func_215456_a(d4, d5, d6, d7, (double)chunkY * d7) / d3;
            d1 += this.field_222569_p.getOctave(i).func_215456_a(d4, d5, d6, d7, (double)chunkY * d7) / d3;

            if (i < 8) {
                d2 += this.field_222570_q.getOctave(i).func_215456_a(OctavesNoiseGenerator.maintainPrecision((double)chunkX * depthSize * d3), OctavesNoiseGenerator.maintainPrecision((double)chunkY * unk1 * d3), OctavesNoiseGenerator.maintainPrecision((double)chunkZ * depthSize * d3), unk1 * d3, (double)coordinateScale * unk1 * d3) / d3;
    ...
    protected void func_222546_a(double[] p_222546_1_, int p_222546_2_, int p_222546_3_, double p_222546_4_, double p_222546_6_, double p_222546_8_, double p_222546_10_, int p_222546_12_, int p_222546_13_) {
        ...
        for(int i = 0; i < this.func_222550_i(); ++i) {
            double d4 = this.useNoises(p_222546_2_, i, p_222546_3_, p_222546_4_, p_222546_6_, p_222546_8_, p_222546_10_);
