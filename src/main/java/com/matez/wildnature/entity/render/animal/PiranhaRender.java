...
import com.matez.wildnature.entity.render.item.PiranhaHeldItemLayer;
...
@OnlyIn(Dist.CLIENT)
public class PiranhaRender extends MobRenderer<PiranhaEntity, PiranhaModel<PiranhaEntity>> {
    public PiranhaRender(EntityRendererManager manager) {
        super(manager, new PiranhaModel<>(), 0.2F);
        this.addLayer(new PiranhaHeldItemLayer(this));

    }

    @Nullable
    @Override
    protected ResourceLocation getEntityTexture(PiranhaEntity entity) {
        if(!entity.isAngry()) {
